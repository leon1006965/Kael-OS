...
void font_draw_string(int x, int y, const char* s, uint8_t color) {
    while (*s) {
...
void font_draw_string_bg(int x, int y, const char* s, uint8_t fg, uint8_t bg) {
    while (*s) {
        font_draw_char_bg(x, y, *s, fg, bg);
        x += 8;
        s++;
    }
}
